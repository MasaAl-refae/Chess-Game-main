function start() {
  ...
  for (var rr = 0; rr < parseInt(rn, 10); rr++) {
    ...
    numberArray.push(newpalce);
    placeofQueens = placeofQueens + newpalce;
    ...
    document.getElementById("game").appendChild(di);
    ...
    for (var r = 0; r < parseInt(rn, 10); r++) {
      ...
      if ((r + rr) % 2) {
        div.className = "gamecell wight";
      } else {
        div.className = "gamecell grey";
      }
      div.style.width = "400px";
      div.style.height = "400px";
      div.id = c;

      document.getElementById(rr).appendChild(div);


      xx = parseInt(newpalce);
      if (r === xx) {
        var i = document.createElement("i");
        i.className = "fa-sharp fa-solid fa-chess-queen fa-bounce fa-xs";
        i.style.color = "#998200";
  ...
  while (1) {
    ...
    if (vv < pv) {
      ...
      for (var i = 0; i < parseInt(rn, 10); i++) {
        for (var ii = 0; ii < parseInt(rn, 10); ii++) {
          //[ii+ Math.pow(parseInt(rn,10) , i)]

          document.getElementById('poq').innerText = document.getElementById('poq').innerText + childrenvalue[ii + parseInt(rn, 10) * i];
          if (childrenvalue[ii + parseInt(rn, 10) * i] === vv && flage === 0)
          {
